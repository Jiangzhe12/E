...
### 每日单词学习（SRS）
- **每日总量上限**（默认 40，设置里可调）：复习优先，新词补满剩余名额；复习积压超出上限时分几天摊平、新词自动让路，单日总量恒定——漏学几天也不会一次性堆成「复习墙」
- **学习日节奏**（默认周一~周四，设置里可调）：休息日不发新词、不计入断签（连续天数跨周末不清零），到期复习顺延到下一个学习日
- 到期判定按「天」而非「时刻」，配合每日封顶；进度会显示 `今日单词 x/40 · 复习 N（还有 M 待后续）`
- 「标记熟悉」后进入间隔复习（1 / 3 / 7 / 14 / 30 天，五段后毕业），复习卡分「还记得 / 忘了」；休息日仍可做到期复习或手动「开一组」
- 可点「学习下一组」在上限之外临时多学一组，进度同步显示在主窗口与桌宠气泡
